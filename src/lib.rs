...
impl LiDARRenderScene {
    ...
    pub async fn get_lidar_returns(&mut self, rc: &RenderContext) {
        ...
        let compute_pipeline = rc.device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("rt"),
            layout: None,
            module: &shader,
    ...
    fn build_blas(&self, rc: &RenderContext) -> BLASScene {
        ...
        let vertex_buf = rc.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Vertex Buffer"),
            contents: bytemuck::cast_slice(&vertex_data),
            usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::BLAS_INPUT,
        });
    
        let index_buf = rc.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
