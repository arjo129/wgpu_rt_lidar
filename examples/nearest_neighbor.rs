...
async fn query_nearest_neighbours(voxel: &DenseVoxel, points: Vec<Vec3>) -> Option<Vec<u32>> {
    ...
    execute_gpu_inner(&device, &queue, voxel, &points).await
}
...
#[tokio::main]
pub async fn main() {
    let mut voxel_grid = DenseVoxel::new(
    ...
    let target = voxel_grid.add_item(VoxelItem {
